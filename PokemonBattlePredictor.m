...
%     stats(winnerid,5:12) - stats(loserid,5:12)
    ...
    statPercent(i,:) = table2array(stats(pokemon1id,5:10))./table2array(stats(pokemon2id,5:10));
...
% Stat difference is 1-2 stats, so we append the outcome to be the last column
statPercent = [statPercent outcome']; 
X = statPercent(:,1:6);
Y = statPercent(:,7);
...
[winner] = battleSim(battles,stats,23,19,mdl) %Wailmer and Charizard
